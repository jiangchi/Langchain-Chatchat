...
chat_box = ChatBox(assistant_avatar=os.path.join(
    "img",
    # llm回答结果的最前面的图标
    "chatchat_icon_blue_square_v2.png"))


def get_messages_history(history_len: int,
...
def dialogue_page(api: ApiRequest, is_lite: bool = False):
    ...
    st.session_state["conversation_ids"].setdefault(chat_box.cur_chat_name,
                                                    uuid.uuid4().hex)
    ...
    if modal.is_open():
        with modal.container():
            cmds = [
                x for x in parse_command.__doc__.split("\n")
    ...
    with st.sidebar:
        ...
        conversation_id = st.session_state["conversation_ids"][
            conversation_name]
        ...
        if (st.session_state.get("prev_llm_model") != llm_model and not is_lite
        ...
        if "prompt_template_select" not in st.session_state:
            st.session_state.prompt_template_select = prompt_templates_kb_list[
                0]
        ...
        if dialogue_mode == "知识库问答":
            ...
            with st.expander("搜索引擎配置", True):
                ...
                se_top_k = st.number_input("匹配搜索结果条数：", 1, 20,
                                           SEARCH_ENGINE_TOP_K)
    ...
    def on_feedback(
        ...
        score_int = chat_box.set_feedback(feedback=feedback,
    ...
    if prompt := st.chat_input(chat_input_placeholder, key="prompt"):
        if parse_command(text=prompt, modal=modal):  # 用户输入自定义命令
            """  重新运行应用是指重新执行应用程序的代码，从而生成新的界面和交互效果。在 Streamlit 中，重新运行应用通常是通过调用 st.rerun() 函数实现的。

            当用户输入自定义命令时，应用程序会检测到该命令并调用 st.rerun()，从而重新运行整个应用程序的代码。这样，应用程序会生成一个新的界面和交互效果，用户可以查看和操作这些新内容。

            例如，如果应用程序有一个计算器界面，用户可以在界面上输入数字并计算结果。如果用户输入了某个命令，应用程序会检测到该命令并重新运行，从而生成一个新的界面，用户可以查看和操作新的计算器界面。

            重新运行应用的效果类似于刷新页面，但与刷新页面相比，重新运行应用可以保留用户输入的信息，例如对话框中的消息记录等。因此，在需要根据用户输入生成新的内容时，使用 st.rerun() 可以更好地满足用户需求。 """
            st.rerun()
        else:
            ...
            if dialogue_mode == "LLM 对话":
                ...
                chat_box.show_feedback(**feedback_kwargs,
                                       ...
                                       kwargs={
                                           ...
                                           message_id,
                                           "history_index":
                                           len(chat_box.history) - 1
                                       })

            elif dialogue_mode == "自定义Agent问答":
                if not any(agent in llm_model
